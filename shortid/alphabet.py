...
class Alphabet(object):
    ORIGINAL = '{0}{1}_-'.format(string.digits, string.ascii_letters)
    ...
    def set(self, characters):
        ...
        if characters_len != original_len:
            raise RuntimeError(
                'Custom alphabet for shortId must be {0} unique characters. You submitted {1} characters: {2}'
                .format(original_len, characters_len, characters)
            )

        characters_set_len = len(set(characters))
        if characters_set_len != characters_len:
            raise RuntimeError(
                'Custom alphabet for shortId must be {0} unique characters. {1} non unique values found.'
                .format(original_len, characters_len - characters_set_len)
            )
